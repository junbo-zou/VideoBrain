...
# get_rope_index is imported dynamically based on processor type (Qwen2VL vs Qwen3VL)
...
def _strip_system_block(text: str) -> str:
    """Remove the first <|im_start|>system...<|im_end|> block from text."""
...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    if processor is not None and "Qwen2VLImageProcessor" in processor.image_processor.__class__.__name__:
        # Support both Qwen2VL and Qwen3VL (Qwen3VL's image_processor inherits from Qwen2VL)
...
class ParallelEnv:
    ...
    def reset(self, prompts, vllm_inputs,n=1,**kwargs):
        ...
        for i in range(len(prompts)):
            ...
            # Extract question from extra_info for agent prompt
