        ...
        for test_data in self.val_dataloader:
            ...
            if "multi_modal_inputs" in test_batch.non_tensor_batch.keys():
                test_gen_batch = test_batch.pop(
                    ...
                    non_tensor_batch_keys=['raw_prompt_ids', 'multi_modal_data', 'origin_multi_modal_data', 'multi_modal_inputs','video_path','fps','total_frames','height','width'],
                )
            else:
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
            ...
            sample_scores.extend(acc_scores)

            reward_extra_infos_dict["total"].extend(total_scores)
            reward_extra_infos_dict["acc"].extend(acc_scores)
            reward_extra_infos_dict["format"].extend(format_scores)
            reward_extra_infos_dict["behavior_bonus"].extend(other_scores)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                #print("batch",batch)
                #print("batch.non_tensor_batch.keys()",list(batch.non_tensor_batch.keys()))
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        try:
                            ...
                            other_tensor = reward_result.get("other_tensor", None)
                            reward_extra_infos_dict = reward_result["reward_extra_info"]
                        except Exception as e:
                            print(f"Error in reward_fn: {e}")
                            reward_tensor, acc_tensor, format_tensor, other_tensor = self.reward_fn(batch)
