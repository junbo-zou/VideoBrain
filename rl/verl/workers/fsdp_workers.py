...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if torch_dtype is None:
            # Use bfloat16 for both actor and ref to enable Flash Attention 2
        ...
        if self.config.actor.get("freeze_vision_tower", False):
            ...
            if vision_tower is not None:
                ...
                if self.rank == 0:
                    num_frozen_params = sum(p.numel() for p in vision_tower.parameters())
                    print(f"[actor model] Vision tower frozen: {num_frozen_params/1e6:.1f}M params set to not trainable.")
            else:
                if self.rank == 0:
                    print("[actor model] freeze_vision_tower=True but no vision tower found.")
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        # Use bfloat16 by default to enable Flash Attention 2
